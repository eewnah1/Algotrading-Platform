...
async function loadExperiments() {
  ...
  div.innerHTML = (exps.slice(0,8)||[]).map(e => `<div class="card"><h3>${e.id}</h3><p>${e.hypothesis}</p><span class="badge ${e.status==='passed'?'ok':e.status==='failed'?'fail':'warn'}">${e.status}</span><pre class="code">${e.code}</pre></div>`).join('');
}
